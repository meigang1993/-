...
- Relic data, runtime, and UI: `src/original/data-relics.js`, `src/original/data-future-relics.js`, `src/original/relics.js`, `src/original/relic-bindings.js`; picker rendering, codex interaction, and event binding live in `src/original/relic-ui-picker.js`, `src/original/relic-ui-codex-interactions.js`, and `src/original/relic-ui-bindings.js` behind `src/original/relic-ui.js`.
...
- Battle intro pause flow: `src/original/battle-line-intro.js`; it is assembled with the speech and caption controllers by `src/original/battle-lines.js`.
...
- Battle audio: `src/original/battle-audio.js` owns the public facade and cue selection, `src/original/battle-audio-samples.js` owns sample loading/media fallback, `src/original/battle-damage-fx.js` owns damage visuals, and `src/original/battle-damage-audio.js` owns synthesized elemental hit audio. These and the animation/effect modules publish together as `battle-presentation`, separate from battle rules, flow/settlement, AI, and UI.
- Role positioning text: `evaluation` fields in data files and canonical single-primary-role `combatRoles` assignments in `src/original/data-combat-roles.js`; rendered beside the name in `src/original/ui-info.js` and in skill tooltips through the `src/original/ui-common.js` compatibility facade. Skill lookup/state and markup live in `src/original/ui-common-skill-model.js` and `src/original/ui-common-skill-view.js` behind `src/original/ui-common-skills.js`; art resolution, card previews, and relic rendering live in the matching `ui-common-*` modules.
...
- Future character portrait assets retained for later updates: `publish/assets/new-portraits/maria.webp` (玛利亚) and `publish/assets/new-portraits/atina.webp` (亚缇娜). These compressed 2048×2048 WebP files are reserved assets only and are not connected to `GameData.characters`, unlock events, or player-facing UI yet.
- Skin roster and fixed skin assets: `src/original/data-skins.js`; ownership, equipment, and runtime selection live in `src/original/skins.js`. Dedicated appearance constraints and skill-effect rules are recorded under the skin sections in this file. 芙萝娅“音速刺客”的战斗与胜利表现由`src/original/flora-sonic-skin-fx.js`和`publish/flora-sonic-skin.css`负责；`src/original/flora-speed-assault.js`、`src/original/flora-skills.js`与`src/original/carlos-skills.js`实现对应技能，`src/original/flora-carlos-skills.js`保留兼容门面；技能结算中的摸牌异常回滚由`src/original/battle-draw-transaction.js`负责。
...
- 28名角色的初始面板与20级生命成长保持不变；攻击/魔力按基础成长的`3x`总量执行，速度按`2.5x`总量执行。没有跨角色共用的20级终值上限；完整初始属性与20级成长总量以角色数据源和`src/original/character-progression.js`为准。
- 2026年8月10日角色调整：芙萝娅基础杀意上限改为`1`，`神速之袭`在准备阶段和结束阶段各限一次，同一回合可分别发动；准备阶段发动仍跳过本回合判定阶段和摸牌阶段，结束阶段发动后芙萝娅翻面并跳过下一个完整回合。`神速之袭`的翻面与击杀摸牌必须等本次攻击动画完成后一次性提交；只有该虚拟【刺杀】首次直接命中的实际生命伤害令实际受伤目标死亡时才摸牌，后续连锁伤害不计，待复活目标不计，最后一名敌人仍须先发放该奖励再进入胜利结算。摸牌中途异常必须回滚牌堆、手牌、随机游标、日志和待播事件，并保留结算事件供后续重试；重复提交或已经替换的旧战斗不得再次摸牌、翻面或回写状态。贝丝妲魔偶基础魔力改为`3`。卡洛斯基础攻击力改为`3`、基础杀意上限改为`1`。
...
- The August 7, 2026 enemy rebalance is derived from the corrected stronger playable-character level curve, current character skills, each enemy's actual skill amplification, and dungeon encounter composition; uploaded monster drafts are not an input. The Ruins Sand City enemy portrait mapping is verified by enemy ID: the sniper uses the attack-drone portrait asset and the attack drone uses the noble-sniper portrait asset. Skills, AI, card resources, group composition, difficulty multipliers, and the separately approved draw bonuses remain unchanged.
...
- Character levels run from `0` through `20`. Required experience for each next level is `100, 160, 240, 340, 470, 620, 800, 1020, 1280, 1580, 1920, 2300, 2720, 3180, 3680, 4100, 4550, 5050, 5600, 6200`, for `45910` total experience from level 0 to 20.
...
- Files: `src/original/battle-lines.js` is the facade; `src/original/battle-speech-controller.js` and `src/original/battle-caption-controller.js` own timed speech and skill captions.
...
- 安倍麦克的【幻影剑舞】在一次战斗回合内至多启动一次；技能开始结算时必须先记录当前回合并清零本回合实体【杀】计数，再执行虚拟杀，避免娜娜莉反击或其他异步恢复链重新进入结束阶段而重复发动。
...
- 手动全体出牌的目标线批次由实际飞牌唯一创建；飞牌期间必须暂停目标线同步并设置战斗级手动群体飞行标记。结算过程中由扫射、【魔王军入侵】等生成的虚拟群体出牌镜像仍可保留飞行和伤害事件，但必须跳过重复目标线；飞牌收尾后清除标记并恢复重绘，不能让预览线、临时群体牌和逐目标牌在异步边界重复绘制同一批目标线。
