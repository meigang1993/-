...
- `力量爆发`（本回合每张实体【杀】倍率递增）显示血甲收紧、巨剑膨胀与粗壮暗红剑柱；`狂战意志`受击或造成伤害时获得血色印记（受击更强），消耗1枚狂战标记代替1点杀意消耗时显示印记炸裂、血雾回流与能量脉冲，并化为本回合攻击血色拖尾（`angelica-berserker-rage-trail`连线，由`battle-damage-utils.js`在攻击时触发）；`猩红暴走`依次播放血甲收束、标记逐枚炸裂、插地冲击波与猩红巨人虚影，每弃置1枚标记触发一圈自脚下上涌的脉冲光环，最终在胸口汇聚成猩红核心后沉入体内。
