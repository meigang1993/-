...
- 技能数据中的`type`是玩法语义，不得为了通过展示审计而改写：`复仇之刃`和`剑盾反攻`必须持续保持`trigger`/`🔵`，分别由玩家响应或敌方 AI 决定是否发动。`tools/skill-audit-presentation-contract-tests.js`必须同时断言这两个触发技身份；技能展示修复只能调整视图映射，不能把触发技改成`passive`。
