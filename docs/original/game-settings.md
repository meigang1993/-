...
- Skin roster and fixed skin assets: `src/original/skins.js`; dedicated appearance constraints and skill-effect rules are recorded under the skin sections in this file. 芙萝娅“音速刺客”的战斗与胜利表现由`src/original/flora-sonic-skin-fx.js`和`publish/flora-sonic-skin.css`负责，技能触发点位于`src/original/flora-carlos-skills.js`；技能结算中的摸牌异常回滚由`src/original/battle-draw-transaction.js`负责。
