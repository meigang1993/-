...
- 安洁莉卡默认战场、角色栏与头像立绘固定使用`./assets/images/angelica-portrait.96fc74ac.webp`，1024×1024方形WebP资源；奥菲莉亚默认战场、角色栏与头像立绘固定使用`./assets/images/ophelia-portrait.10278309.webp`，2048×2048方形WebP压缩资源。两者均通过`src/original/data-characters-extra.js`引用。
...
- 伊迪斯当前默认战场与角色详情立绘使用压缩资源`./assets/images/pursuer-edis.8d1fc755.8d1fc755.webp`，保持`2048×2048`方形构图；旧`pursuer-edith.webp`不再保留。
...
- 贝丝妲魔偶的`锁魂镰刀`仅在她使用实体单体【杀】造成生命值伤害后触发；实体与转换单体杀可触发，虚拟单体杀、全体杀与无目标杀不能触发。
...
- 皮肤 ID：`wendy_benevolent_teacher`；品质固定为史诗，兑换价格固定为10精华宝珠，仅属于温蒂；`wendy_default`保留原立绘并允许随时切回。默认立绘使用正式压缩素材`publish/assets/images/wendy-portrait.f262b034.f262b034.webp`；慈爱教师素材固定为`publish/assets/generated/wendy-benevolent-teacher-desert-school.bcfc0372.webp`，专属动态特效标识固定为`wendy-teacher`。
...
- 艾尔拉娜等级特殊立绘 ID 固定为`elrana_level_10_special`；品质固定为特殊，价格为0，`unlockLevel`固定为10，使用正式压缩素材`publish/assets/generated/elrana-level-10-special.812773fa.webp`。达到Lv.10后自动加入已拥有皮肤并允许装备、放大查看和作为正式战斗立绘；测试战斗不得无视等级和拥有状态试用，未解锁时显示等级锁定占位。该立绘与“堕落医师”的购买、装备和专属特效互相独立。
