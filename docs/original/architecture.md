...
| Battle rules | `src/original/battle*.js`, including the dedicated `src/original/battle-status-cards.js` subsystem, shared `src/original/battle-draw-feedback.js`, `src/original/character-skill-access.js`, and dedicated character/enemy skill modules; Wendy and Cadicis implementations live in `src/original/wendy-skills.js` and `src/original/cadicis-skills.js` behind `src/original/wendy-cadicis-skills.js`, while Ace and Nanali implementations live in `src/original/ace-skills.js` and `src/original/nanali-skills.js` behind `src/original/ace-nanali-skills.js` |
