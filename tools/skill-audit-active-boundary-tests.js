...
module.exports = ({ assert, card, unit }) => {
  ...
  definitions.forEach(def => {
    ...
    const allyFemale = unit(`female-${def.flags[0]}`, "ally", { gender: "female" });
    ...
    assert(access.canResolve(window.state, actor, targets[def.target], skillCard, () => true),
    ...
    if (skillCard.speedAssault) {
      battle.phase = 6;
      assert(access.canResolve(window.state, actor, enemy, skillCard, () => true),
    ...
    assert(!access.canActor({ ...battle, allies: [allyMale, allyFemale] }, actor, skillCard),
    ...
    if (deadTarget !== actor) {
      deadTarget.hp = 0;
      assert(!access.canResolve(window.state, actor, deadTarget, skillCard, () => true),
        `${def.name} must reject a defeated target`);
      deadTarget.hp = deadTarget.maxHp;
      const outside = { ...deadTarget, uid: `outside-${def.flags[0]}` };
      assert(!access.canResolve(window.state, actor, outside, skillCard, () => true),
  ...
  definitions.forEach(def => {
    ...
    const battle = {
      phase: 4,
      allies: [actor, unit(`ally-${def.flags[0]}`, "ally", { gender: "male" })],
  ...
  assert(!access.canActor(corruptBattle, corruptActor, makeCard(definitions[0])),
