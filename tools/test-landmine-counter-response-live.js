// 实战：持雷者用「看破」类响应牌（counter 路径）时，地雷是否触发
// 对照：持雷者用「闪」响应（已知触发路径）
...
const inject = (scene) => `(() => {
  ...
  window.state.settings.manualResponse = true;   // 手动模式：确保窗口可见并可点击
  const findCard = n => {
  ...
  if (${JSON.stringify(scene)} === "counter") {
...
(async () => {
  ...
  const out = {};
  for (const scene of ["counter", "dodge"]) {
    ...
    try {
      ...
      // 手动响应是两步：先 pick 选牌（触发 render 重建 DOM），再 use 确认
