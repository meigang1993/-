window.UICommonSkills = deps => {
  ...
  function skillsOf(unit, fallback = "passive") {
    ...
    const baseSkills = window.GuestCharacterSkills?.normalizeSkills?.(unit, storedSkills)
      || storedSkills;
  ...
  function activeSkillName(unit, card) {
    ...
    return skillsOf(unit).some(skill =>
      skill?.type === "active" && skill.name === card.name)
      ? card.name : "";
  }
  ...
  function skillTitle(unit) {
    const skills = skillsOf(unit)
      ...
      .map(skillTip).join("\n\n") || "暂无技能";
    return [combatRoleText(unit), rolePositionText(unit), skills].filter(Boolean).join("\n\n");
  }
  ...
  function skillState(actor, skill, battle) {
    ...
    if (skill?.type !== "active") return { usable: false, reason: "此技能会自动生效" };
  ...
  function skillName(skill, index = 0, clickable = false, selected = false,
    ...
    const state = stateInfo || skillState(
      battle ? window.BattleSystem?.active?.(battle) : null,
      skill,
      battle
    );
  ...
  return {
    activeSkillName, skillsOf, skillText, skillSummary, skillTitle,
    combatRolesOf, combatRoleText, combatRoleBadges,
    rolePositionText, skillState, skillName,
  };
};
