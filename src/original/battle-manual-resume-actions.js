window.BattleManualResumeActions = deps => {
  const {
  ...
  async function resumeInterruptedActions(state, onStep, inherited) {
    ...
    while (current() && state.battle && !state.battle.locked
      ...
      if (battle.manualDodgeResume) {
        await deps.hitResume.resolveManualResume(state, onStep, current);
        continue;
      }
      if (battle.demonInvasionResume) {
        await deps.hitResume.resumeDemonInvasion(state, onStep, current);
        continue;
      }
  ...
  return { resumeInterruptedActions, resumeComboAttack, resumeGreenGatling };
};
