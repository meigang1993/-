window.bindBattleActionButtons = function bindBattleActionButtons() {
  ...
  document.querySelector("[data-counter-trigger-use]")?.addEventListener("click", e => { e.stopPropagation(); BattleActionGuard.run("触发技发动失败", async ({ state: actionState, isCurrent }) => { await BattleSystem.resolveCounterTrigger(actionState, true, render); if (!isCurrent()) return false; render(); persist({ battleOperation: true }); }, { control: e.currentTarget }); });
  document.querySelector("[data-counter-trigger-skip]")?.addEventListener("click", e => { e.stopPropagation(); BattleActionGuard.run("触发技跳过失败", async ({ state: actionState, isCurrent }) => { await BattleSystem.resolveCounterTrigger(actionState, false, render); if (!isCurrent()) return false; render(); persist({ battleOperation: true }); }, { control: e.currentTarget }); });
