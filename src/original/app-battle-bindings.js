window.bindBattleActionButtons = function bindBattleActionButtons() {
  ...
  document.querySelectorAll("[data-landmine-rps-choice]").forEach(b => b.onclick = e => { e.stopPropagation(); runStagedAction("地雷猜拳处理失败", ({ state: actionState, isCurrent }) => { window.RuinsEnemySkills?.landmineRps?.resolveChoice?.(actionState, b.dataset.landmineRpsChoice); render(); if (!isCurrent()) return false; persist({ battleOperation: true }); }, b); });
  document.querySelector("[data-landmine-rps-result-confirm]")?.addEventListener("click", e => { e.stopPropagation(); runStagedAction("地雷猜拳结果确认失败", ({ state: actionState, isCurrent }) => { window.RuinsEnemySkills?.landmineRps?.confirm?.(actionState); render(); if (!isCurrent()) return false; persist({ battleOperation: true }); }, e.currentTarget); });
  document.querySelector("[data-landmine-rps-skip]")?.addEventListener("click", e => { e.stopPropagation(); runStagedAction("地雷猜拳跳过失败", ({ state: actionState, isCurrent }) => { window.RuinsEnemySkills?.landmineRps?.skip?.(actionState); render(); if (!isCurrent()) return false; persist({ battleOperation: true }); }, e.currentTarget); });
