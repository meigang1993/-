window.VillaCollectionUI = ({ U, cardTypeLabel }) => {
  ...
  function cardCodex(state) {
    ...
    return `<div class="card-codex-panel"><div class="codex-head"><h2>卡牌图鉴</h2><span class="tag">已收集种类：${counts.ownedTypes}/${counts.codexTypes}</span><span class="tag">实体卡总数：${cards.length}</span></div><div class="card-codex-grid">${items}</div>${picked ? codexDetail(picked, owned.has(picked.name)) : ""}<div class="actions"><button data-card-codex="1">返回</button></div></div>`;
  }
