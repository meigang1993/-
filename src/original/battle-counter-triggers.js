window.BattleCounterTriggers = (() => {
  function open(state, payload) {
    ...
    if (!battle || battle.counterTrigger || !unit || unit.hp <= 0
      || !payload?.skill || !payload?.sourceUid) return false;
    battle.counterTrigger = {
      skill: payload.skill, unitUid: unit.uid, unitName: unit.name,
      sourceUid: payload.sourceUid, targetUid: payload.targetUid || payload.sourceUid,
      mode: payload.mode || "single", count: Math.max(1, payload.count || 1),
    };
    battle.locked = true;
  ...
  function resolve(state, use, api = {}) {
    ...
    if (use && source?.hp > 0) {
      const resolvers = {
        ...
        "复仇反击": () => window.BertisGerlotSkills?.resolveRevengeTrigger?.(state, source, target, prompt.count, api),
        "护母反击": () => window.BertisGerlotSkills?.resolveRevengeTrigger?.(state, source, target, prompt.count, api),
        "贝尔蒂丝受伤反击": () => window.BertisGerlotSkills?.resolveRevengeTrigger?.(state, source, target, prompt.count, api),
  ...
  return { open, clear, resolve, pending: battle => !!battle?.counterTrigger };
})();
