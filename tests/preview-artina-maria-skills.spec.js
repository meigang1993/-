...
test("玛利亚·神数咒语：出牌计数递增并摸牌，回合结束清零", async ({ page }) => {
  ...
  // 标记随「使用的牌数」一同清零，攻魔加成 = 标记数；回合结束同样清零。
  // 第1张: marks=1, 达目标1 → 摸1张，目标升为2，标记清零，加成=0
  // 第2张: marks=1, 未达目标2 → 不摸，加成=1
  // 第3张: marks=2, 达目标2 → 摸2张，目标升为3，标记清零，加成=0
  // 第4张: marks=1, 未达目标3 → 不摸，加成=1
